...
class PythonSetupTests(unittest.TestCase):
    ...
    def test_metrics_resolved_with_runtime_pins(self):
        ...
        self.assertEqual(installs, [[
            '-m', 'pip', 'install', '--quiet',
            '-r', str(REPO / 'setup/requirements-runtime.txt'),
    ...
    def test_existing_invalid_venv_explains_recovery(self):
        ...
        self.assertEqual(result.returncode, 1)
        self.assertIn('move it aside or choose a new runtime directory', result.stderr)
    ...
    def test_new_conda_uses_canonical_version(self):
        ...
        result = self.run_setup('--conda', 'rcwm')
        self.assertEqual(result.returncode, 73, result.stderr)
        self.assertEqual((self.root / '.venv').resolve(), prefix)
        self.assertIn(['create', '-y', '-q', '-n', 'rcwm', 'python=3.12'], self.calls())
