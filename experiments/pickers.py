"""统一的各方法成品选图规则(图与表共用)。返回 (路径或 None, 是否已到该方法自己的停机态)"""
...
def _ok(f):
    ...
    if any(e in f.lower() for e in ('audit','inventory','evidence','material','mask')): return False  # 贴图/证据/审计目录不是成品
...
def ours(root,SC):
    ...
    if delivered:
        # 交付了但没叫 final.png:先信 part.json 自己声明的成品渲染,再取与参考同画幅、名字不是对照/侧视/审计的最新整帧
        ...
        # 再认几个智能体常用的固定相机成品名(精确匹配,不猜:猜会拿到环视/方位图);都没有就当未收官,等人眼认
    ...
    # 未收官:取最新的整体渲染(round/assembled/handoff),不取侧视/对照片
...
def _viga_best(base):
    ...
    for r in sorted((int(x) for x in os.listdir(base) if x.isdigit()),reverse=True):
        # VIGA 按 Blender 里的相机名存图:通常是 Camera.png,智能体改过名就是别的 *.png
    ...
    return None,-1
def viga(root,SC):
    # 正式:pilot2-<SC>(只跑本景,high);停机态=官方 runner 正常收尾
    ...
    # 过渡:旧的串景发射里本景的最高轮(优先同名发射)——只在默认的 pilot2 批次里启用;
    # 显式指定 RCWM_VIGA_TESTID 的批次(如 v2-{scene})必须严格只认自己的输出,不回落到别的发射
    cands=[]
    ...
    # 正式跑收官就用它;没收官时只有轮数不低于旧结果且至少 5 轮才替换,免得用第 1 轮草稿顶替
...
def i2t(root,SC):
    # 用户裁定(2026-09-09):只认原生流程自己跑完的 city-full r1;其余九景以隔离工作区重跑为准(两段:原生→门禁停后强行做完)
    ...
    for n in ('final-render.png','render-hires.png'):
        c=[f for f in glob.glob(f'{d}/**/{n}', recursive=True) if _img_ok(f) and 'img2threejs/' not in f.replace(d+'/','')]  # 它自家命名的成品,不论尺寸
